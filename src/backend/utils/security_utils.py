...
def log_registration_attempt(username, email, ip_address, success, message=""):
    """Log registration attempts"""
    log_security_event(
        "REGISTRATION_ATTEMPT", 
        f"Registration {'success' if success else 'failed'} for {username} ({email}) - {message}",
        ip_address=ip_address
    )

def log_admin_action(admin_id, action, target, details, ip_address=None):
    """Log admin actions"""
    log_security_event(
        "ADMIN_ACTION",
        f"Admin action: {action} on {target} - {details}",
        user_id=admin_id,
        ip_address=ip_address
    )
...
SECURITY_HEADERS = {
    ...
    'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' https://code.jquery.com https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://cdn.datatables.net; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://cdn.jsdelivr.net https://cdnjs.cloudflare.com https://cdn.datatables.net; font-src 'self' https://fonts.gstatic.com https://cdnjs.cloudflare.com; img-src 'self' data:; media-src 'self' data:; connect-src 'self' https://cdn.jsdelivr.net",
