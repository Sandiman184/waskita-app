...
from utils.utils import admin_required, log_user_activity, generate_activity_log, generate_otp
from markupsafe import escape
from utils.security_utils import log_registration_attempt, log_admin_action, log_rate_limit_exceeded, log_security_event, SecurityValidator
...
def register_request():
    ...
    try:
        ...
        if pending_request:
            flash('There is already a pending registration request for this username or email', 'warning')
            return render_template('auth/register_request.html')
    ...
    if request.method == 'POST':
        try:
            ...
            if registration_request.status != 'pending':
                flash('This registration request has already been processed', 'warning')
                return redirect(url_for('otp.pending_registrations'))
            ...
            if action == 'approve':
                ...
                if existing_user:
                    flash('Username already taken. Registration cannot be approved.', 'error')
                    return redirect(url_for('otp.pending_registrations'))
                
                # Check if email already exists
                existing_email = User.query.filter_by(email=registration_request.email).first()
                if existing_email:
                    flash('Email already taken. Registration cannot be approved.', 'error')
                    return redirect(url_for('otp.pending_registrations'))
                ...
                flash(f'Registration for user {registration_request.username} successfully approved!', 'success')
                ...
                flash(f'Registration for user {registration_request.username} rejected.', 'info')
...
def verify_first_login_otp():
    ...
    if 'first_login_user_id' not in session or 'first_login_otp' not in session:
        flash('OTP session invalid or expired. Please login again.', 'error')
    ...
    try:
        ...
        if datetime.utcnow() > otp_expires:
            flash('OTP code expired. Please login again to get a new code.', 'error')
            # Clear session
    ...
    if request.method == 'POST':
        ...
        if not otp_code:
            flash('OTP code cannot be empty!', 'error')
        ...
        generate_activity_log(
            action='first_login',
            description=f'First login successful with OTP verification for user: {user.username}',
            user_id=user.id,
...
def resend_first_login_otp():
    ...
    try:
        ...
        if not user_id:
            return jsonify({
                'success': False, 
                'message': 'First login session not found. Please login again.'
            }), 400
        ...
        # Send new OTP email using services
